...
async def attended_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    attended_events = [reg for reg in registrations if str(reg['User ID']) == str(user_id) and datetime.strptime(reg['Date'], '%d/%m/%Y') < datetime.strptime(today, '%d/%m/%Y')]
...
async def upcoming_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    upcoming_events = [reg for reg in registrations if str(reg['User ID']) == str(user_id) and datetime.strptime(reg['Date'], '%d/%m/%Y') >= datetime.strptime(today, '%d/%m/%Y')]
...
def main() -> None:
    ...
    app.add_handler(CommandHandler("attended", attended_command))
